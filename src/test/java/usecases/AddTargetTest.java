...
class AddTargetTest {
    ...
    public void addFourTargets() {
        ...
        assertEquals(2, targets.targetList.size()); //asserts that 2 targets are now added to the target list.
        AddTarget addTarget2 = new AddTarget(new Date(), (float) 60, targets);
        addTarget2.addTarget();
        assertEquals(3, targets.targetList.size()); //asserts that 3 targets are now added to the target list.
